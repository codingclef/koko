...
초기 데이터를 병렬로 불러오며 탭 셸을 더 일찍 표시하자, iOS PWA가 잘못 계산한 `100dvh` 높이가 그대로 고정되는 경우가 생겼다. 이때 하단 내비게이션 아래에 빈 영역이 남았고, 사용자 조작으로 viewport가 다시 계산되면 정상으로 돌아왔다. 셸 밖으로 빠져나오는 리마인더 상세 오버레이는 상태바와 홈 인디케이터 영역도 직접 피하지 못했다.
