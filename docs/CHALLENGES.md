...
같은 브라우저의 다른 탭, 또는 다른 기기에서 리마인더와 캘린더 변경이 즉시 반영되지 않았다.
...
리마인더 아이템/목록을 optimistic UUID로 먼저 렌더링한 뒤, 그 임시 ID를 그대로 삭제/수정/정렬에 사용하면 서버 row와 ID가 맞지 않아 아이템이 "삭제된 것처럼 보였다가 다시 살아나는" 문제가 생겼다.
...
const realItem = await addReminderItem(...)
setItems((prev) =>
...
특히 리마인더 DB 물리 테이블인 `shopping_items`, `calendar_members` 주변에서 이런 문제가 반복됐다.
...
캘린더, 리마인더, 설정을 각각 별도 페이지로 운용할 때 탭 전환 시 언마운트/재마운트가 발생해 fetch와 스피너가 반복됐다.
모바일 앱 같은 연속성이 깨졌다.
...
캘린더는 고정 화면인데, iOS Safari에서는 `touchmove`를 막아도 body 레벨 스크롤이 계속 살아 있었다.
...
## 12. iOS PWA에서 초기 탭 높이와 safe area가 어긋나던 문제

### 과제

성능 개선에서 캘린더 메타데이터 로딩을 초기 splash 조건에서 제외하자, iOS PWA의 viewport가 안정되기 전에 `TabsShell`이 마운트되기 시작했다. 이때 순간적으로 작게 계산된 `100dvh` 높이가 유지되어 하단 내비게이션 아래에 빈 영역이 남았고, 사용자 조작으로 viewport가 다시 계산되면 정상으로 돌아왔다. 셸 밖으로 빠져나오는 리마인더 상세 오버레이는 상태바와 홈 인디케이터 영역도 직접 피하지 못했다.
