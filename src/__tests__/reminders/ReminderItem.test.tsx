...
const mockItem: ReminderItemType = {
  id: 'item-1',
  list_id: 'list-1',
  created_by: 'user-1',
  name: '우유',
...
describe('ReminderItem', () => {
  ...
  it('체크 버튼 클릭 시 onCheck가 호출된다', () => {
    ...
    render(<ReminderItem item={mockItem} listType="strikethrough" onCheck={onCheck} onDelete={jest.fn()} onRename={jest.fn()} />)
  ...
  it('체크된 아이템은 취소선이 표시된다 (strikethrough 방식)', () => {
    ...
    render(<ReminderItem item={checkedItem} listType="strikethrough" onCheck={jest.fn()} onDelete={jest.fn()} onRename={jest.fn()} />)
  ...
  it('이름 수정 후 Enter 키로 onRename이 호출된다', () => {
    ...
    render(<ReminderItem item={mockItem} listType="strikethrough" onCheck={jest.fn()} onDelete={jest.fn()} onRename={onRename} />)
    fireEvent.click(screen.getByText('우유'))
  ...
  it('draggable=true이면 드래그 핸들이 표시된다', () => {
    render(<ReminderItem item={mockItem} listType="strikethrough" onCheck={jest.fn()} onDelete={jest.fn()} onRename={jest.fn()} draggable />)
