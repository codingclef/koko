...
import type { ReminderList, ItemPreview } from '@/lib/reminder-lists'
...
describe('ReminderListCard', () => {
  ...
  it('삭제 버튼 클릭 시 다이얼로그가 나타난다', () => {
    render(<ReminderListCard list={mockList} onDelete={jest.fn()} onRename={jest.fn()} onOpen={mockOnOpen} />)
  ...
  it('다이얼로그에서 삭제 클릭 시 onDelete가 호출된다', () => {
    ...
    render(<ReminderListCard list={mockList} onDelete={onDelete} onRename={jest.fn()} onOpen={mockOnOpen} />)
  ...
  it('이름 수정 후 Enter 키로 onRename이 호출된다', () => {
    ...
    render(<ReminderListCard list={mockList} onDelete={jest.fn()} onRename={onRename} onOpen={mockOnOpen} />)
    fireEvent.click(screen.getByText('이마트'))
